...
def check_keyup_events(event, al_settings, screen, ship, bullets):
    '''响应松开'''
    if event.key == pygame.K_RIGHT:
...
def check_events(al_settings, screen, stats, play_button, ship, aliens, bullets,sb):
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_keyup_events(event, al_settings, screen, ship, bullets)
        elif event.type == pygame.MOUSEBUTTONDOWN:
            mouse_x, mouse_y = pygame.mouse.get_pos()
            check_play_button(al_settings, screen, stats, play_button, ship, aliens, bullets, mouse_x, mouse_y, sb)

def check_play_button(al_settings, screen, stats, play_button, ship, aliens, bullets, mouse_x, mouse_y,sb):
    """在玩家单击Play按钮时开始新游戏"""
    ...
    if not button_clicked and not stats.game_active:
        ...
        ship.center_ship()
    if len(aliens) == 0:
        # 删除现有子弹， 加快游戏进度，并创建一批新的外星人
        bullets.empty()
        al_settings.increase_speed()
        creat_fleet(al_settings, screen, ship, aliens)

def updata_screen(al_settings, screen, ship, bullets, aliens, play_button,stats, sb):
    '''更新屏幕上图像，并切换到新屏幕'''
    # 每次循环时重置屏幕
...
def updata_bullets(al_settings, screen, ship, aliens, bullets, stats, sb):
    """更新子弹的位置，并删除已消失的子弹"""
    # 更新子弹的位置
    bullets.update()
...
def ship_hit(al_settings, stats, screen, ship, aliens, bullets, sb):
    """响应被外星人撞到的飞船"""
    if stats.ship_left - 1 > 0:   # -1 表示不包括当前飞船的生命值
        # 将ships_left减1
        stats.ship_left -= 1
        # 更新记分牌
        sb.prep_ships()
...
def update_aliens(al_settings, stats, screen, ship, aliens, bullets,sb):
    """检查是否有外星人位于屏幕边缘，并更新整群外星人的位置"""
    check_fleet_edges(al_settings, aliens)
    aliens.update()
    # 检测外星人与飞船之间的碰撞
    if pygame.sprite.spritecollideany(ship, aliens):
        ship_hit(al_settings, stats, screen, ship, aliens, bullets, sb)
    # 检测是否有外星人到达屏幕底端
